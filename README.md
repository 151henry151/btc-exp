...
- **`ESPLORA_CACHE_TTL_MS`** — In **`MIX_ENV=prod`**, defaults to **60000** (60 seconds): dashboard Esplora calls (**recent blocks**, **mempool**, **fee estimates**) are cached in ETS. Concurrent cache misses for the same key **single-flight** (one **`recent_blocks`** pagination run at a time) so many LiveView mounts do not multiply traffic. Set **`0`** to disable. Public APIs (e.g. Blockstream) often return **HTTP 429** under heavy traffic; mitigations include your own **`ESPLORA_BASE_URL`**, a higher TTL, slower UI polling (ship defaults), or **`DATA_SOURCE=rpc`**.
- **`ESPLORA_MIN_REQUEST_INTERVAL_MS`** — When **`DATA_SOURCE=esplora`**, **`MIX_ENV=prod`** defaults to **450** ms if unset: Esplora HTTP runs through **`BitcoinexExplorer.EsploraHttpGate`**, which serializes requests and enforces a minimum gap between **starting** each call (aligned with reference nginx rate limits below). Set **`0`** to disable (e.g. private Esplora).
