defmodule BitcoinexExplorerWeb.ExplorerLiveTest do
  ...
  test "home includes QR scan hook on home card", %{conn: conn} do
    ...
    assert html =~ "home-qr-scan"
    assert html =~ ~s(data-after-scan="submit-search")
    assert html =~ ~s(aria-label="Scan QR code")
    refute html =~ "decode-qr-scan"
    refute html =~ ~s(data-after-scan="decode-change")
  end
  ...
  test "auto-detects Lightning invoice by ln prefix", %{conn: conn} do
    ...
    html = render_submit(view, "home_action", %{"q" => inv, "action" => "decode"})
  ...
  test "auto-detects PSBT by base64 magic", %{conn: conn} do
    ...
    html = render_submit(view, "home_action", %{"q" => psbt, "action" => "decode"})
  ...
  test "shows Bech32 checksum error for invalid bc1 address, not generic decode", %{conn: conn} do
    ...
    html = render_submit(view, "home_action", %{"q" => bad, "action" => "decode"})
  ...
  test "shows no error for empty input", %{conn: conn} do
    ...
    html = render_submit(view, "home_action", %{"q" => "", "action" => "decode"})
  ...
  test "shows friendly error for invalid invoice", %{conn: conn} do
    ...
    html = render_submit(view, "home_action", %{"q" => "lnbc1notvalid", "action" => "decode"})
