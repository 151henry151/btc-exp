...
- Add **`LightningSnapshot`** to write daily **`snapshot-YYYY-MM-DD.json`** files (prod default **`priv/data/lightning_snapshots/`**; **`LIGHTNING_SNAPSHOT_DIR`** / **`disable`** in **`.env.production.example`**); document roadmap for past→present graph animation in **`docs/lightning-graph-history.md`** and **`README.md`**.

## [1.0.2] - 2026-05-06

- **`LightningGraph`** (**`hooks.js`**): run **`d3.force`** synchronously with **`sim.tick()`** until **`alpha ≤ alphaMin`**, then paint edges and nodes once (no animated **`restart`**); pin **`fx`/`fy`** after settle.
